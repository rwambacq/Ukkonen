	...
	for (int i = 0; i < len; i++) {
		...
		while ((!done) && (info->remaining > 0)) {
			...
			if (info->active_pointer->active_length == 0) {
				...
				TreeEdge* active_edge = info->active_pointer->active_node->edges[info->active_pointer->active_edge];
			...
			if (al_aanwezig) {
				...
				if (info->active_pointer->active_length == 0) {
					...
					if (prev_added == NULL) {
						...
						if (info->active_pointer->active_edge != '\0') {
							prev_added_edge = info->active_pointer->active_node->edges[info->active_pointer->active_edge];
						}
					} else {
					...
					if (info->active_pointer->active_node->suffix_link) {
						...
						if (info->active_pointer->active_edge != '\0') {
							...
							while (info->active_pointer->active_length >= range) {
								...
								active_edge = info->active_pointer->active_node->edges[info->active_pointer->active_edge];
...
int main(int argc, char* argv[]) {
	...
	SuffixTree* test_boom = ukkonen(string, size+1);
